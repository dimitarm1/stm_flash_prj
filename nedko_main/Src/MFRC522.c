...
void MFRC522_PCD_WriteRegister(uint8_t reg, uint8_t value)
{
  ...
  digitalWrite(mfrc522.m_CS_port, mfrc522.m_CS_pin,LOW);

  // MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
  shiftOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin, reg & 0x7E);
  shiftOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin, value);

  /* Release SPI Chip MFRC522 */
  digitalWrite(mfrc522.m_CS_port, mfrc522.m_CS_pin,HIGH);
...
void MFRC522_PCD_WriteRegister_2(uint8_t reg, uint8_t count, uint8_t *values)
{
  ...
  {
	shiftOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin,values[index]);
  }
...
uint8_t MFRC522_PCD_ReadRegister(uint8_t reg)
{
  ...
  shiftOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin,0x80 | reg);

  // Read the value back. Send 0 to stop reading.
  value = shiftInOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_MISO_port, mfrc522.SPI_MISO_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin,0);
...
void MFRC522_PCD_ReadRegister_2(uint8_t reg, uint8_t count, uint8_t *values, uint8_t rxAlign)
{
  ...
  shiftOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin,address);
  ...
  while (index < count)
  {
    if ((index == 0) && rxAlign) // Only update bit positions rxAlign..7 in values[0]
    {
      ...
      uint8_t value = shiftInOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_MISO_port, mfrc522.SPI_MISO_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin,address);
    ...
    {
      // Read value and tell that we want to read the same address again.
      values[index] = shiftInOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_MISO_port, mfrc522.SPI_MISO_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin,address);
    }

    index++;
  }
  // Read the final byte. Send 0 to stop reading.
  values[index] = shiftInOut(mfrc522.SPI_MOSI_port, mfrc522.SPI_MOSI_pin, mfrc522.SPI_MISO_port, mfrc522.SPI_MISO_pin, mfrc522.SPI_CLK_port, mfrc522.SPI_CLK_pin,0);
...
uint8_t MFRC522_PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result)
{
  ...
  MFRC522_PCD_SetRegisterBits(FIFOLevelReg, 0x80);      // FlushBuffer = 1, FIFO initialization
  MFRC522_PCD_WriteRegister_2(FIFODataReg, length, data); // Write data to the FIFO
  ...
  MFRC522_PCD_WriteRegister(CommandReg, PCD_Idle);
...
{
  mfrc522.m_CS_pin = csPin;
  mfrc522.m_CS_port = cs_port;

  mfrc522.m_RESET_pin = resetPin;
  mfrc522.m_RESET_port = reset_port;

  mfrc522.SPI_MOSI_pin = mosiPin;
  mfrc522.SPI_MOSI_port = mosi_port;

  mfrc522.SPI_MISO_pin = misoPin;
  mfrc522.SPI_MISO_port = miso_port;

  mfrc522.SPI_CLK_pin = clkPin;
  mfrc522.SPI_CLK_port = clk_port;
  /* Reset MFRC522 */
  digitalWrite(mfrc522.m_RESET_port, mfrc522.m_RESET_pin,LOW);
  HAL_Delay(10);
  digitalWrite(mfrc522.m_RESET_port, mfrc522.m_RESET_pin,HIGH);
  ...
  MFRC522_PCD_WriteRegister(TPrescalerReg, 0xA9); // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 => f_timer=40kHz, ie a timer period of 25us.
  ...
  MFRC522_PCD_WriteRegister(RFCfgReg, (0x07<<4)); // Set Rx Gain to max
                                         ...
                                         uint8_t    checkCRC)
{
  ...
  MFRC522_PCD_SetRegisterBits(FIFOLevelReg, 0x80);            // FlushBuffer = 1, FIFO initialization
  MFRC522_PCD_WriteRegister_2(FIFODataReg, sendLen, sendData);  // Write sendData to the FIFO
  ...
  {
	  MFRC522_PCD_SetRegisterBits(BitFramingReg, 0x80);      // StartSend=1, transmission of data starts
  }
  ...
  if (backData && backLen)
  {
    n = MFRC522_PCD_ReadRegister(FIFOLevelReg);           // Number of bytes in the FIFO
    if (n > *backLen)
    {
    ...
    *backLen = n;                       // Number of bytes returned
    MFRC522_PCD_ReadRegister_2(FIFODataReg, n, backData, rxAlign);    // Get received data from FIFO
...
uint8_t MFRC522_PICC_REQA_or_WUPA(uint8_t command, uint8_t *bufferATQA, uint8_t *bufferSize)
{
  ...
  MFRC522_PCD_ClrRegisterBits(CollReg, 0x80);
...
uint8_t MFRC522_PICC_HaltA()
{
  ...
  if (result == STATUS_OK)
  {
    ...
    result = MFRC522_PCD_TransceiveData(buffer, sizeof(buffer), NULL, 0, 0, 0, 0);
...
uint8_t MFRC522_PCD_Authenticate(uint8_t command, uint8_t blockAddr, MIFARE_Key *key, Uid *uid)
{
  ...
  return MFRC522_PCD_CommunicateWithPICC(PCD_MFAuthent, waitIRq, &sendData[0], sizeof(sendData), 0, 0, 0, 0, 0);
...
void MFRC522_PCD_StopCrypto1()
{
  // Clear MFCrypto1On bit
	MFRC522_PCD_ClrRegisterBits(Status2Reg, 0x08); // Status2Reg[7..0] bits are: TempSensClear I2CForceHS reserved reserved   MFCrypto1On ModemState[2:0]
} // End PCD_StopCrypto1()
...
uint8_t MFRC522_MIFARE_Read(uint8_t blockAddr, uint8_t *buffer, uint8_t *bufferSize)
{
  ...
  return MFRC522_PCD_TransceiveData(buffer, 4, buffer, bufferSize, NULL, 0, true);
} // End MIFARE_Read()
...
uint8_t MFRC522_MIFARE_Write(uint8_t blockAddr, uint8_t *buffer, uint8_t bufferSize)
{
  ...
  result = MFRC522_PCD_MIFARE_Transceive(cmdBuffer, 2, 0);
  if (result != STATUS_OK)
  {
    return result;
  }
  ...
  // Adds CRC_A and checks that the response is MF_ACK.
  result = MFRC522_PCD_MIFARE_Transceive(buffer, bufferSize, 0);
  if (result != STATUS_OK)
  {
    return result;
  }
...
uint8_t MFRC522_MIFARE_TwoStepHelper(uint8_t command, uint8_t blockAddr, uint32_t data)
{
  ...
  result = MFRC522_PCD_MIFARE_Transceive((uint8_t *) &data, 4, true);
  if (result != STATUS_OK)
  {
    return result;
  }
...
uint8_t MFRC522_MIFARE_Transfer(uint8_t blockAddr)
{
  ...
  // Adds CRC_A and checks that the response is MF_ACK.
  return MFRC522_PCD_MIFARE_Transceive(cmdBuffer, 2, 0);
} // End MIFARE_Transfer()
...
uint8_t MFRC522_PCD_MIFARE_Transceive(uint8_t *sendData, uint8_t sendLen, uint8_t acceptTimeout)
{
  ...
  uint8_t validBits = 0;
  result = MFRC522_PCD_CommunicateWithPICC(PCD_Transceive, waitIRq, cmdBuffer, sendLen, cmdBuffer, &cmdBufferSize, &validBits, 0, 0);
 ...
 */
char* MFRC522_PICC_GetTypeName(uint8_t piccType)
{
 ...
 */
uint8_t MFRC522_PICC_ReadCardSerial(void)
{
  uint8_t result = MFRC522_PICC_Select(&mfrc522.uid, 0);
